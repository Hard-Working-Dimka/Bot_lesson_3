...
def reply_from_dialogflow(event, vk_api, project_id):
    message = detect_intent_texts(project_id, f'vk-{event.user_id}', event.text, "ru")
    if message:
    ...
    try:
        ...
        for event in longpoll.listen():
            if event.type == VkEventType.MESSAGE_NEW and event.to_me:
                reply_from_dialogflow(event, vk_api, project_id)
